...
class TourView(ctk.CTkFrame):
    ...
    def create_header(self):
        lbl = ctk.CTkLabel(self, text="Quản lý Danh mục Tour", font=ctk.CTkFont(size=24, weight="bold"))
    ...
    def create_toolbar(self):
        ...
        self.entry_search = ctk.CTkEntry(toolbar, placeholder_text="Tìm tên tour, điểm đến...", width=250)
        self.entry_search.pack(side="left", padx=(0, 10))
        
        self.combo_filter = ctk.CTkComboBox(toolbar, values=["Tất cả", "Hoạt động", "Tạm dừng"], width=120, command=lambda _: self.load_data())
        self.combo_filter.pack(side="left", padx=10)
        
        ctk.CTkButton(toolbar, text="Tìm", width=60, command=self.load_data).pack(side="left")

        # Nút chức năng
        ctk.CTkButton(toolbar, text="Xóa", fg_color="#e74c3c", hover_color="#c0392b", width=80, command=self.delete_tour).pack(side="right", padx=5)
        ctk.CTkButton(toolbar, text="Sửa", fg_color="#f39c12", hover_color="#d35400", width=80, command=self.edit_tour).pack(side="right", padx=5)
        ...
        ctk.CTkButton(toolbar, text="+ Thêm Tour", fg_color="#2ecc71", hover_color="#27ae60", width=120, command=self.add_tour).pack(side="right", padx=5)
